...
impl TypeChecker {
    ...
    fn type_check_stmt(&mut self, stmt: ResolvedStmt) -> Result<TypedStmt, TypeError> {
        let typed_stmt = match stmt {
            ...
            ResolvedStmt::VarDecl(res, ident, typ) => {
                self.identifier_types.insert(ident.clone(), typ.clone());
                let type_info = VariableTypeInfo::new(typ.clone(), res);
                TypedStmt::VarDecl(type_info, ident, typ)
            }
